...
var FileName,Content:ansistring;
    fm:byte;
    f:file;
begin
 ResultValue.ValueType:=bvtUNDEFINED;
...
const BoolStr:array[boolean] of ansistring=('false','true');
var FileName,Content:ansistring;
  ...
  if CountArguments>0 then begin
   ...
   if FindFirst(String(FileName),faAnyFile or faDirectory,SearchRec)=0 then begin
    ...
    repeat
     ...
     Content:=Content+'{';
     Content:=Content+'"name":'+AnsiString(BESENJSONStringQuote(BESENUTF8ToUTF16(BESENEncodeString(AnsiString(SearchRec.Name),BESENLocaleCharset,UTF_8))))+',';
     Content:=Content+'"size":'+AnsiString(IntToStr(SearchRec.Size))+',';
     Content:=Content+'"time":'+AnsiString(BESENFloatToStr(BESENDateTimeToBESENDate(FileDateToDateTime(SearchRec.Time))))+',';
 ...
 try
  if CountArguments>0 then begin
   ...
   if ioresult=0 then begin
    ...
    ResultValue:=TBESEN(Instance).Execute(BESENConvertToUTF8(Content));
    {$i-}closefile(f);{$i+}
    if ioresult=0 then begin
    end;
    filemode:=fm;
 ...
 try
  try
   if not DoExit then begin
    TBESEN(Instance).InjectObject('console',BESENUTF16ToUTF8(BESENObjectConsoleSource));
   end;
  except
  ...
  if length(FileName)>0 then begin
   ...
   while not DoExit do begin
    ...
    readln(ss);
    try
     v:=TBESEN(Instance).Execute(BESENUTF16ToUTF8(ss));
     if not DoExit then begin
      Print(v);
      v.ValueType:=bvtUNDEFINED;
      v.Obj:=nil;
