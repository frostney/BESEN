...
begin
 ...
 while assigned(Range) do begin
  if assigned(TBESEN(RegExp.Instance).RegExpDebugOutputHook) then begin
   TBESEN(RegExp.Instance).RegExpDebugOutputHook(TBESEN(RegExp.Instance),'[0x'+TBESENUTF8STRING(IntToHex(Range.Lo,8))+',0x'+TBESENUTF8STRING(IntToHex(Range.Hi,8))+']',true);
  end;
  Range:=Range.Next;
 end;
end;
...
begin
 ...
 a:=0;
 while (PC>=0) and (PC<ByteCodeLen) do begin
  Line:='0x'+TBESENUTF8STRING(IntToHex(PC,8))+': ';
  Opcode:=ByteCode[PC];
  inc(PC);
  case Opcode of
   breoFAIL:begin
   ...
   breoCHAR:begin
    GetParams1;
    Line:=Line+'CHAR '+TBESENUTF8STRING(IntToStr(i));
   end;
   breoZERO:begin
    GetParams1;
    Line:=Line+'ZERO '+TBESENUTF8STRING(IntToStr(i));
   end;
   breoREACH:begin
    GetParams2;
    Line:=Line+'REACH '+TBESENUTF8STRING(IntToStr(i))+', '+TBESENUTF8STRING(IntToStr(i2));
   end;
   breoNREACH:begin
    GetParams2;
    Line:=Line+'NREACH '+TBESENUTF8STRING(IntToStr(i))+', '+TBESENUTF8STRING(IntToStr(i2));
   end;
   breoSTART:begin
    GetParams1;
    Line:=Line+'START '+TBESENUTF8STRING(IntToStr(i));
   end;
   breoEND:begin
    GetParams1;
    Line:=Line+'END '+TBESENUTF8STRING(IntToStr(i));
   end;
   breoUNDEF:begin
    GetParams2;
    Line:=Line+'UNDEF '+TBESENUTF8STRING(IntToStr(i))+', '+TBESENUTF8STRING(IntToStr(i2));
   end;
   breoMARK:begin
    GetParams1;
    Line:=Line+'MARK '+TBESENUTF8STRING(IntToStr(i));
   end;
   breoFDIST:begin
    GetParams1;
    Line:=Line+'FDIST '+TBESENUTF8STRING(IntToStr(i));
   end;
   breoRDIST:begin
    GetParams3;
    Line:=Line+'RDIST '+TBESENUTF8STRING(IntToStr(i))+', '+TBESENUTF8STRING(IntToStr(i2))+', '+TBESENUTF8STRING(IntToStr(i3));
   end;
   breoMNEXT:begin
    GetParams2A;
    Line:=Line+'MNEXT '+TBESENUTF8STRING(IntToStr(i))+', '+TBESENUTF8STRING(IntToStr(i2))+', 0x'+TBESENUTF8STRING(IntToHex(a,8));
   end;
   breoRNEXT:begin
    GetParams2A;
    Line:=Line+'RNEXT '+TBESENUTF8STRING(IntToStr(i))+', '+TBESENUTF8STRING(IntToStr(i2))+', 0x'+TBESENUTF8STRING(IntToHex(a,8));
   end;
   breoGOTO:begin
    GetParamsA;
    Line:=Line+'GOTO 0x'+TBESENUTF8STRING(IntToHex(a,8));
   end;
   breoGS:begin
    GetParamsA;
    Line:=Line+'GS 0x'+TBESENUTF8STRING(IntToHex(a,8));
   end;
   breoNS:begin
    GetParamsA;
    Line:=Line+'NS 0x'+TBESENUTF8STRING(IntToHex(a,8));
   end;
   breoGF:begin
    GetParamsA;
    Line:=Line+'GF 0x'+TBESENUTF8STRING(IntToHex(a,8));
   end;
   breoNF:begin
    GetParamsA;
    Line:=Line+'NF 0x'+TBESENUTF8STRING(IntToHex(a,8));
   end;
   breoAS:begin
    GetParamsA;
    Line:=Line+'AS 0x'+TBESENUTF8STRING(IntToHex(a,8));
   end;
   breoAF:begin
    GetParamsA;
    Line:=Line+'AF 0x'+TBESENUTF8STRING(IntToHex(a,8));
   end;
   breoBOL:begin
    Line:=Line+'BOL';
   end;
   ...
   breoBACKREF:begin
    GetParams1;
    Line:=Line+'BACKREF '+TBESENUTF8STRING(IntToStr(i));
   end;
   else begin
    raise EBESENInternalError.Create('Internal error: 201002250323-0000');
   end;
...
function TBESENRegExp.Execute(PC:longint;const Input:TBESENString;var State:TBESENRegExpState;var RemainTimeOutSteps:int64):boolean;
 procedure Print(Data:ansistring);
 begin
  if (TBESEN(Instance).RegExpDebug<>0) and assigned(TBESEN(Instance).RegExpDebugOutputHook) then begin
   TBESEN(Instance).RegExpDebugOutputHook(TBESEN(Instance),TBESENUTF8STRING(Data),false);
  end;
 end;
 procedure PrintLn(Data:ansistring);
 begin
  if (TBESEN(Instance).RegExpDebug<>0) and assigned(TBESEN(Instance).RegExpDebugOutputHook) then begin
   TBESEN(Instance).RegExpDebugOutputHook(TBESEN(Instance),TBESENUTF8STRING(Data),true);
  end;
 end;
